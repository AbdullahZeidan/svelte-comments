...
import type { ThreadData, CommentData, User } from './types';
...
function createThread() {
	const { subscribe, update } = writable<ThreadData>({
		comments: [
			{
				id: '1',
				content:
	...
	return {
		...
		addComment: ({ data, afterCommentId }: { data: CommentData; afterCommentId?: string }) => {
			if (!afterCommentId) {
				update((thread) => ({
					comments: thread.comments.concat(data)
				}));
				return;
			}
				...
				newComments.forEach((cmnt, idx) => {
					...
					cmnt.replies?.forEach((reply, i) => {
						if (reply.id === commentId) {
							...
							cmnt.replies![i] = { ...reply, content: newContent };
						}
					});
				});
				return {
		...
		deleteComment: (commentId: string) => {
			update((thread) => {
				...
				newComments = newComments.filter((cmnt) => cmnt.id !== commentId);
